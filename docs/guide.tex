...
\includegraphics[width=0.75\linewidth,viewport=145 55 625 450,clip]{../figures/coarser.png}
...
\includegraphics[width=0.75\linewidth,viewport=145 55 625 450,clip]{../figures/finer.png}
...
function {\tt tune}). We also provide an example in Jupyter Notebook,
please see Appendix \ref{subsec:jupyter-notebook}.
...
\subsection{A Jupyter Notebook Example for Appendix \ref{sec:astro}}
\label{subsec:jupyter-notebook}
To make the discussion easier, we show only the example from Appendix \ref{sec:astro} here.
For examples with other data sets, please see \href{https://colab.research.google.com/github/ntumlgroup/libsvm-guide/blob/master/guide.ipynb}{this link}.
\input{guide_example.tex}
...
an anonymous user)
% Georges Bonga at
% Beuth Hochschule f\"{u}r Technik
% Berlin.
It is available at 
...
677,399 instances.
\begin{alltt}$ time liblinear-2.47/train -c 0.25 -v 5 rcv1_test.binary 
...
features 54.  We run \liblinear with {\tt -s 1} (default) and {\tt -s
  2}.
\begin{alltt}$ time liblinear-2.47/train -c 4 -v 5 -s 2 covtype.libsvm.binary.scale 
